...
export default function Reports() {
  ...
  async function loadSales() {
    const { data } = await api.get("/admin/reports/sales", {
      params: { from, to, source: source || undefined },
    });
    setReport(data);
  }

  async function loadShifts() {
    const { data } = await api.get("/admin/reports/shifts", {
      params: { from, to },
    });
    setShifts(data);
  }
      ...
      <div className="p-8">
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          <div>
            ...
            <p className="text-ngip-muted text-sm">
              Sales performance and cashier shift history.
            </p>
          </div>
        ...
        <div className="flex gap-2 mb-5">
          <button
            ...
            className={`text-sm px-3 py-1.5 rounded-lg ${tab === "sales" ? "bg-ngip-accent text-ngip-bg font-semibold" : "text-ngip-muted hover:bg-white/5"}`}
          >
            Sales
          </button>
          <button
            onClick={() => setTab("shifts")}
            className={`text-sm px-3 py-1.5 rounded-lg ${tab === "shifts" ? "bg-ngip-accent text-ngip-bg font-semibold" : "text-ngip-muted hover:bg-white/5"}`}
          >
            Shift History
          </button>
        </div>
        ...
        {tab === "sales" && report && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
              <StatCard
                label="Total Revenue"
                value={peso(report.totalRevenue)}
                accent
              />
              <StatCard label="Total Orders" value={report.totalOrders} />
              <StatCard
                label="Avg. Order Value"
                value={peso(report.avgOrderValue)}
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div className="bg-ngip-panel border border-white/5 rounded-2xl p-5">
                ...
                {Object.entries(report.revenueBySource).map(([k, v]) => (
                  <div
                    key={k}
                    className="flex justify-between text-sm py-1.5 border-b border-white/5 last:border-0"
                  >
                ...
                {Object.keys(report.revenueBySource).length === 0 && (
                  <div className="text-ngip-muted text-sm">No data.</div>
                )}
              </div>
              <div className="bg-ngip-panel border border-white/5 rounded-2xl p-5">
                <h2 className="font-display font-bold text-sm mb-3">
                ...
                {Object.keys(report.revenueByMethod).length === 0 && (
                  <div className="text-ngip-muted text-sm">No data.</div>
                )}
              </div>
            </div>

            <div className="bg-ngip-panel border border-white/5 rounded-2xl">
              <div className="px-5 py-3 border-b border-white/5 font-display font-bold text-sm">
                Top Products
              </div>
                ...
                {shifts.map((s) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-ngip-muted whitespace-nowrap">
                      {new Date(s.opened_at).toLocaleString("en-PH")}
                    </td>
                    <td className="px-4 py-3 text-ngip-muted whitespace-nowrap">
                      {s.closed_at
                        ? new Date(s.closed_at).toLocaleString("en-PH")
                        : "—"}
                    </td>
                    <td className="px-4 py-3">{peso(s.opening_cash)}</td>
                    <td className="px-4 py-3">
                      {s.closing_cash != null ? peso(s.closing_cash) : "—"}
                    </td>
                    <td
                      className={`px-4 py-3 ${Number(s.cash_variance) < 0 ? "text-ngip-accent2" : Number(s.cash_variance) > 0 ? "text-green-400" : ""}`}
                    >

...
export default function Inventory() {
  ...
  async function loadProducts() {
    const { data } = await api.get('/admin/inventory', { params: showLowStockOnly ? { low_stock: 'true' } : {} });
    setProducts(data);
  }
  ...
  useEffect(() => { loadProducts(); }, [showLowStockOnly]);
  useEffect(() => { if (tab === 'history') loadMovements(); }, [tab]);

  async function handleStockSubmit(payload) {
    setError(''); setSuccess('');
    try {
      const endpoint = modal.mode === 'restock'
        ? `/admin/inventory/${modal.product.id}/restock`
        : `/admin/inventory/${modal.product.id}/adjust`;
      await api.post(endpoint, payload);
      setSuccess(`${modal.mode === 'restock' ? 'Restocked' : 'Adjusted'} ${modal.product.name}.`);
        ...
        <div className="flex items-center justify-between mb-6 shrink-0">
          <div>
            ...
            <p className="text-ngip-muted text-sm">
              ...
              {lowStockCount > 0 && <span className="text-ngip-accent2"> · {lowStockCount} low-stock item(s)</span>}
            </p>
          </div>
        </div>
            ...
            <div className="flex-1 overflow-y-auto bg-ngip-panel border border-white/5 rounded-2xl">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {products.map((p) => {
                    ...
                    return (
                      <tr key={p.id} className="border-b border-white/5 last:border-0 hover:bg-white/5">
                        ...
                        <td className={`px-4 py-3 font-medium ${low ? 'text-ngip-accent2' : ''}`}>
                          {p.stock_qty} {low && <span className="text-xs">⚠ low</span>}
                        </td>
                        <td className="px-4 py-3 text-ngip-muted">{p.low_stock_threshold}</td>
                        <td className="px-4 py-3 text-right space-x-3">
                          <button onClick={() => setModal({ open: true, mode: 'restock', product: p })} className="text-ngip-accent hover:underline">
                            Restock
                          </button>
                          <button onClick={() => setModal({ open: true, mode: 'adjust', product: p })} className="text-ngip-muted hover:underline">
                            Adjust
                          </button>
                        </td>
                      </tr>
                  ...
                  {products.length === 0 && (
                    <tr><td colSpan={5} className="px-4 py-8 text-center text-ngip-muted">No products found.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
              ...
              <tbody>
                {movements.map((m) => (
                  <tr key={m.id} className="border-b border-white/5 last:border-0">
                    <td className="px-4 py-3 text-ngip-muted whitespace-nowrap">{new Date(m.createdAt).toLocaleString('en-PH')}</td>
                    <td className="px-4 py-3">{m.Product?.name || '—'}</td>
                    <td className="px-4 py-3 capitalize text-ngip-muted">{m.type.replace(/_/g, ' ')}</td>
                    <td className={`px-4 py-3 font-medium ${m.quantity_change < 0 ? 'text-ngip-accent2' : 'text-green-400'}`}>
                      {m.quantity_change > 0 ? '+' : ''}{m.quantity_change}
                    </td>
                    <td className="px-4 py-3">{m.resulting_stock}</td>
                    <td className="px-4 py-3 text-ngip-muted">{m.actor?.name || 'System'}</td>
                    <td className="px-4 py-3 text-ngip-muted">{m.note || '—'}</td>
                  </tr>
                ))}
                {movements.length === 0 && (
                  <tr><td colSpan={7} className="px-4 py-8 text-center text-ngip-muted">No stock movements yet.</td></tr>
                )}
              </tbody>
            </table>
          </div>
